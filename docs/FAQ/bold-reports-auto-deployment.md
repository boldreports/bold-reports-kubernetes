# Bold Reports Auto Deployment on Kubernetes Service

This section allows you to deploy [Bold Reports](https://www.boldreports.com/) in Kubernetes without manual activation of licensing and configuring startup from the browser. This also allows us to customize the branding using environment variables.
...
* [Deploy Bold Reports using Kubectl](bold-reports-auto-deployment.md#deploy-bold-reports-using-kubectl)
* [Deploy Bold Reports using Helm](bold-reports-auto-deployment.md#bold-reports-auto-deployment-using-helm)
   

# Deploy Bold Reports using Kubectl

The following steps help you to deploy Bold Reports using kubectl in a Kubernetes cluster. You can create a Kubernetes cluster on cloud cluster providers (GKE, AKS, and EKS). After completing cluster creation, connect to it, and you can download the configuration files [here](../deploy/auto-deployment.zip). This directory includes configuration YAML files, which contain all the configuration settings needed to deploy Bold Reports on the Kubernetes cluster.
...
## Steps for Bold Reports auto deployment using kubectl.

The following links explain Bold Reports Kubernetes deployment in specific cloud environments:

 * [Azure Kubernetes Service(AKS)](bold-reports-auto-deployment.md#bold-reports-on-microsoft-azure-kubernetes-service)
 * [Amazon Elastic Kubernetes Service(EKS)](bold-reports-auto-deployment.md#bold-reports-on-amazon-elastic-kubernetes-service)
 * [Google Kubernetes Engine (GKE)](bold-reports-auto-deployment.md#bold-reports-on-google-kubernetes-engine)


### Bold Reports on Microsoft Azure Kubernetes Service

1. Download the deployment file from [here](https://raw.githubusercontent.com/boldreports/boldreports-kubernetes/main/deploy/auto-deployment/deploy_aks.yaml) to deploy Bold Reports on AKS.
...
7. Enter the variable information needed to complete the auto-deployment in `deploy_aks.yaml` file, as shown in the following image.
    ...
    * Enter the Bold Reports license key, user, and database server details.
        ...
        | -------------                 |----------| ------------- |
        |`BOLD_SERVICES_UNLOCK_KEY`|Yes|License key of Bold Reports|
        ...
        |`BOLD_SERVICES_DB_USER`|Yes|Username for the database server<br /><br />Please refer to [this documentation](https://help.boldreports.com/embedded-reports/faq/what-are-         the-database-permissions-required-to-set-up-bold-reports-embedded/) for information on the user's permissions.|
        ...
        <details>
          ...
          type: Opaque
          stringData:
            BOLD_SERVICES_UNLOCK_KEY: "@332e332e30fgfTa4NmxTdRataMFgre/GC5AyCj+BHVoCO4ax6M61s=eyJFbWFpbCI6InN1YmJpcmFtYW5peWFuLnRAc3luY2Z1c2lvrjb20iLCJQcm9kdWN0cyI6"
          ---
          apiVersion: v1
          kind: Secret
          metadata:  
          ...
          stringData:
            ...
            BOLD_SERVICES_USER_EMAIL: "admin@boldreports.com"
          ...
          type: Opaque
          stringData:
            # Type of database server can be used for configuring the Bold Reports. Eg: mssql, mysql and postgresql
            ...
            # https://help.boldreports.com/embedded-reports/faq/what-are-the-database-permissions-required-to-set-up-bold-reports-embedded/
            BOLD_SERVICES_DB_USER: "boldreports@boldreports-docker"
    ...
    *  If you want to use custom values for branding, enter the branding image and site identifier variable details. Otherwise Bold Reports will take the default values.
...
17. By default, all client libraries for Bold Reports will be installed in Kubernetes. However, you can overwrite them by specifying the required libraries as a comma-separated list in the environment variable noted from the above link.
...
1. Download the deployment file [here](https://raw.githubusercontent.com/boldreports/boldreports-kubernetes/main/deploy/auto-deployment/deploy_eks.yaml) to deloy Bold Reports on AKS.
...
7. Enter the variable information needed to complete the auto-deployment in **deploy_gke.yaml** file, as shown in the following image.
    ...
    * Enter the Bold Reports license key, user, and database server details.
        ...
        <details>
          ...
          type: Opaque
          stringData:
            BOLD_SERVICES_UNLOCK_KEY: "@332e332e30fgfTa4NmxTdRataMFgre/GC5AyCj+BHVoCO4ax6M61s=eyJFbWFpbCI6InN1YmJpcmFtYW5peWFuLnRAc3luY2Z1c2lv5jb20iLCJQcm9kdWN0cyI6"
          ---
          apiVersion: v1
          kind: Secret
          metadata:  
...
# Bold Reports Auto Deployment using Helm

This chart installs [Bold Reports](https://www.boldreports.com/) on Kubernetes. You can create Kubernetes cluster in cloud cluster providers(GKE,AKS and EKS). Please follow the below documentation for Bold Reports deployment in a specific cloud environments.
...
**Note:** Note the [Ingress IP address](docs/pre-requisites.md#get-ingress-ip) and map it with a DNS to craft values.yaml when installing Bold Reports with helm chart.
...
Run the following command to create the namespace where the Bold Reports resources will be deployed in the Kubernetes cluster. The default namespace is <i>bold-services</i>.
...
<table>
    ...
    <tr>
      ...
      <td>
       ...
       Please refer to <a href='configuration.md#persistent-volume'>this</a> section to know more on how to set Persistant Volumes for Bold Reports.
      </td>
    </tr>
    <tr>
      <td>
    ...
    <tr>
      ...
      <td>
       By default autoscaling is enabled in Bold Reports. Please refer to <a href='configuration.md#auto-scaling'>this</a> section to configure autoscaling in Bold Reports.
      </td>
    </tr>
    <tr>
      <td>
      ...
      <td>
       Please refer to <a href='configuration.md#bing-map-widget'>this</a> section to configure Bing Map Widget in Bold Reports.
      </td>
    </tr>
    <tr>
      <td>
...
This following environment variables helps you to deploy Bold Reports in Kubernetes without manually activating licensing and configuring startup from the browser.
...
<table>
   ...
   <tr>
    ...
    <tr>
      ...
      <td>
       Username for the database server<br /><br />Please refer to [this documentation](https://help.boldreports.com/embedded-reports/faq/what-are-the-database-permissions-required-to-set-up-bold-reports-embedded/) for information on the user's permissions.
      </td>
    </tr>
    <tr>
      <td>
...
<details>
          <summary>
            ...
            ## Secrets for Bold Reports Auto Deployment.
            # This section helps you to deploy Bold Reports in Kubernetes without manually activating licensing and configuring startup from the browser.  
            ...
            databaseServerDetails:
              ...
              # Name of the Database Server
              dbHost: "boldreports-docker.postgres.database.azure.com"
              ...
              # Please refer to this documentation for information on the user's permissions.
              # https://help.boldreports.com/embedded-reports/faq/what-are-the-database-permissions-required-to-set-up-bold-reports-embedded/
              dbUser: "boldreports@boldreports"
...
Run the following command to get the latest version of the Bold Reports helm chart.

```console
helm repo update
```

Run the following command to apply changes in your Bold Reports release or upgrade Bold Reports to the latest version.
